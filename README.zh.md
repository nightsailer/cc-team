...
# 统一上下文接力（通过 RelayContext 自动分派模式）
...
```
CLI（cli.py）
  └─ 上下文接力（_context_relay.py, _relay_context.py, _relay_executor.py,
...
插件层（hooks/session_start.py, hooks/stop.py, hooks/statusline.py, hooks/_common.py）
```
...
# CLI: 重启进程（退出旧进程 + 启动新进程 + 自动恢复 Agent）
# cct --team-name my-project team restart
